    ...
    this.marcacaoService.getAllMarcacoes()
      .subscribe(
        (data: MarcacaoDTOListar[]) => {
          // Filtra as marcações cujos serviços possuem dataAnterior e horaAnterior diferentes de vazio
          this.marcacoes = data.filter(marcacao => 
           (marcacao.utilizadorId == this.userService.getUserId()) &&  
            marcacao.servicosMarcados.some(servico => 
              servico.dataAnterior != null && servico.horaAnterior != null
            )
          );
          

          // Atualiza a contagem de marcações após a filtragem
          this.marcacaoCount =this.marcacoes.filter(marcacao => !marcacao.status).length;
  
          // Opcional: Log das marcações filtradas para depuração
